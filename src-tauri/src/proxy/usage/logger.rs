...
impl UsageSemantic {
    fn from_log(log: &RequestLog, input_token_semantics: i64) -> Self {
        Self {
            ...
            reasoning_output_tokens: log.usage.reasoning_output_tokens,
        ...
        let affected_rows = conn
            .execute(
                ...
                rusqlite::params![
                    ...
                    log.usage.reasoning_output_tokens,
            ...
            |row| {
                Ok((
                    ...
                    UsageSemantic {
                        ...
                        reasoning_output_tokens: row.get::<_, i64>(7)? as u32,
...
mod tests {
    ...
    fn test_log_request() -> Result<(), AppError> {
        ...
        let (count, request_model, reasoning_tokens): (i64, String, i64) = conn
            .query_row(
                "SELECT COUNT(*), request_model, reasoning_output_tokens
