...
fn sync_codex_sidebar_usage_file(
    ...
    loop {
        ...
        match insert_sidebar_usage(db, &event) {
    ...
    update_sync_state(db, &path_key, modified, processed_offset)?;
...
fn insert_sidebar_usage(db: &Database, event: &SidebarUsageEvent) -> Result<bool, AppError> {
    let usage = event
        .token_usage()
